...
def hierarchical_kmeans_resampling(X, k_list, T, m, num_init=10, random_state=42):
    """
    Bản sửa lỗi: r_t được tính tự động, không dùng r_t_list nữa.
    """
    print(f"new hkm")
    current_input = X
    centroids = None
    ...
    for t in range(T):
        # Lấy số cụm cho tầng hiện tại
        k_t = k_list[t]
        
        # --- CHIẾN THUẬT R_T ĐỘNG: Lấy 50% số điểm ---
        # Tính toán r_t dựa trên số lượng input hiện có của tầng
        r_t = max(1, int(0.5 * len(current_input) // k_t))
        
        print(f"--- Level {t+1}/{T} (k={k_t}, r_t tự động={r_t}) ---")
        ...
        # Centroids tầng này là input tầng sau
